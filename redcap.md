...
8. **Variable-level metadata + validation fixes (2026-06-12):**
   ...
   - Validation workflow: `SIDECAR_DUMP_DIR=/tmp/dump go test ./app/plugin/impl/redcap2/ -run TestDumpSidecarsForValidation` writes sample sidecars for pyshacl/mlcroissant runs. Note: the hosted DDI-CDI context (ddi-cdi.github.io) currently contains stray git conflict markers; strip them before local RDF parsing.
